function [c4nods,c75nods,chord,s_pan,h,Cm0_y,normals,mac,S] = geo(AR,TR,N,DE25,ETIP,A0p,CM0p,CDP,YF_pos,CF_ratio,DE_flap,FlapCorr,WS)
...
delta_Cm0_flap = -0.5*sin(th)*(1-cos(th))*deg2rad(eta); % delta_CM0
...
% coorLead = zeros(3, 100);
% coorLead = c4nods;
% 
% coorTrai = zeros(3, 100);
% coorTrai = c4nods;
% 
% for i = 1:N
%     coorLead(1,:) = coorLead(1,:) + chord(i)/2;
%     coorTrai(1,:) = coorTrai(1,:) - chord(i)/2;
% end
% 
% coor = [coorLead coorTrai];
% coor(2,:) = WS.*coor(2,:);
% 
% % Plot the physical wing
% figure()
% plot3(coor(1,:), coor(2,:), coor(3,:))
% grid on
% title('Wing parametrization')
% xlabel('x coordinate')
% ylabel('y coordinate')
% zlabel('z coordinate')
